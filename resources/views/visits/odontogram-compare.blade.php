...
@section('page-description', $visit->patient->name)

@section('content')
    <div class="mb-4">
        <a href="{{ route('visits.show', $visit) }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-slate-700">
            <i class="fa-solid fa-arrow-left"></i> Kembali ke Pemeriksaan
        </a>
    </div>

    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <p class="text-xs text-slate-400">Pemeriksaan Saat Ini</p>
            <p class="mt-1 text-sm font-semibold text-slate-900">{{ $current->examination_date->translatedFormat('d F Y') }}</p>
            <p class="text-xs text-slate-500">{{ $current->doctor->name }}</p>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            @if ($previous)
                ...
                <p class="text-xs text-slate-500">{{ $previous->doctor->name }}</p>
            @else
